...
#define GRALLOC_ARM_NUM_FDS_WITHOUT_SHARE_ATTR 1

#define NUM_INTS_IN_PRIVATE_HANDLE(numFds) ((sizeof(struct private_handle_t) - sizeof(native_handle)) / sizeof(int) - (numFds))
...
enum
{
    ...
    GRALLOC_USAGE_CAMERA_BUFFER  =  0xC0000000,
    GRALLOC_USAGE_CAPTURE_SCREEN =  0x60000000,
...
struct private_handle_t
{
	...
	enum
	{
		...
		PRIV_FLAGS_USES_PHY          = 0x00000010
	};
	    ...
	    , phyaddr(0)
	{
		...
		numFds = GRALLOC_ARM_NUM_FDS_WITHOUT_SHARE_ATTR;
		numInts = NUM_INTS_IN_PRIVATE_HANDLE(numFds);
	}
	...
	static int validate(const native_handle *h)
	{
		...
		if (!h || h->version != sizeof(native_handle) || h->numInts != (int)NUM_INTS_IN_PRIVATE_HANDLE(h->numFds) ||
		    (h->numFds != GRALLOC_ARM_NUM_FDS && h->numFds != GRALLOC_ARM_NUM_FDS_WITHOUT_SHARE_ATTR) || hnd->magic != sMagic)
		{
			return -EINVAL;
		}
